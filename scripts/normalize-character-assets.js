  ...
  function walkDirectory(directory) {
    fs.readdirSync(directory, { withFileTypes: true }).forEach((entry) => {
      ...
      if (
        ...
        files.push(
          path.relative(CHARACTER_DIR, absolutePath).split(path.sep).join("/")
        );
      }
    });
  }

  walkDirectory(CHARACTER_DIR);
  files.sort();
...
function isHair1ShoeFile(fileName) {
  ...
  return (
    fileName.startsWith("hair1/") &&
    /(?:^|_)shoes[12](?:_|\.png$)/.test(baseName)
  );
}
...
async function processCharacter(fileName, backupDir) {
  ...
  const isHair2 = fileName.startsWith("hair2/");
